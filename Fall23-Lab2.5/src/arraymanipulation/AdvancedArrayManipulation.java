...
public class AdvancedArrayManipulation {
    ...
    public static void main(String[] args) {
        ...
        // 1. Sort the array in a scending order.
    ...
    public static int[] removeAtIndex(int[] numbers, int index) {
        ...
        for(int i = 0; i < numbers.length - index - 1; i++) {
            newArray[index + i] = numbers[index + i + 1];
        }
        return newArray;
    }
    ...
    public static int getMedian(int[] numbers) {
        int[] newNumbers = cloneArray(numbers);
        sortAscending(newNumbers);
        int len = newNumbers.length;
        if (len % 2 != 0) {
        	return newNumbers[len / 2];
        }else {
        	return ((newNumbers[(len / 2) - 1] + newNumbers[len / 2])/2);
        }
